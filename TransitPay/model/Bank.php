...
return $jwt;
    ...
    $query = "INSERT INTO " . $this->table . " SET bank_account_number= :bank_account_number, firstname= :firstname, lastname= :lastname, phone_number= :phone_number, email= :email, password= :password, balance= :balance, api_key= :api_key, position= :position, arrears= :arrears";
    $stmt = $this->con->prepare($query);
    //clean data
...
public function adminlogin($email,$password,$role){
    $query = "SELECT COUNT(*) FROM " . $this->table . " WHERE email = :email AND password = :password AND position = :role ";
    $stmt = $this->con->prepare($query);
        //bind id to placeholder
    $stmt->bindParam(':email', $this->email);
    $stmt->bindParam(':password', $this->password);
