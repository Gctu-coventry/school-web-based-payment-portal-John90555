		...
		function submitForm1(){
			...
			if(account_number != '' && password != ''){
				...
				<p class="spinner" id="spinner"><img src="../images/Hourglass.gif" alt="Computer man"></p></div>');
					...
					switch (response){
    					case "true":
							$('.replaced').replaceWith("<div class='right column'><h1>ENTER OTP</h1><div class='error'></div>\
	  <div class='userInput'>\
        <input name ='one' class='otpinput' type='text' data-maxlength='1'  oninput='this.value=this.value.slice(0,this.dataset.maxlength)' id='ist'/>\
        <input name ='two' type='text' data-maxlength='1' oninput='this.value=this.value.slice(0,this.dataset.maxlength)' id='sec'  class='otpinput'/>\
        <input name ='three' type='text' data-maxlength='1' oninput='this.value=this.value.slice(0,this.dataset.maxlength)' id='third'  class='otpinput'/>\
        <input name ='four' type='text' data-maxlength='1' oninput='this.value=this.value.slice(0,this.dataset.maxlength)' id='fourth'  class='otpinput'/>\
        <input name ='five' type='text' id='fifth' data-maxlength='1' oninput='this.value=this.value.slice(0,this.dataset.maxlength)'  class='otpinput'/>\
	...
	function balance(){
		...
		function submit_otp(){
			...
			var six = $('input[name=six]').val();
                ...
                $.ajax({url: "../includes/submit_otp.php", type:'POST',data: formData, dataType: "text", success: function(response){	
					console.log(response);
					switch (response){
    					case "atrue":
		...
		var int_p_id =  parseInt(p_id);
				...
				switch(response){
					case 'true':
						...
						<div>REFUND REQUEST WAS SUCCESSFUL</div>\
                		</div>\
